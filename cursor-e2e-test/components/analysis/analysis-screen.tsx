...
import pokecardMock from "@/app/assets/mocks/pokecard.jpg"
...
/** Figma 27055:277 — blur + peach gradient band (394px tall on 760px-tall reference hero). */
const BLUR_BAND_H = Math.round((394 * HERO_H) / 760)

function HeroPeachBlurOverlay() {
  return (
    <div
      className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-b from-transparent to-[#ffddc7] backdrop-blur-[3.277px]"
      style={{
        height: BLUR_BAND_H,
        WebkitMaskImage:
...
const BACK_TOP = Y(32)
const BACK_INSET = 24
const HOME_BAR_TOP = Y(735)
const CANCEL_BOTTOM = Y(462)
...
export function AnalysisScreen() {
  ...
  const sheetStyle: CSSProperties = {
    top: HERO_H,
    height: ARTBOARD_H - HERO_H,
  }

  return (
    <div
      className="relative mx-auto w-full max-w-[440px] shrink-0 overflow-hidden bg-white"
      style={{ height: ARTBOARD_H, minHeight: ARTBOARD_H }}
      data-name="Forth screen"
    >
      {/* Hero — image + blur (Figma 27055:275 region) */}
      <div
        className="absolute inset-x-0 top-0 overflow-hidden bg-neutral-200"
        style={{ height: HERO_H }}
      >
        <Image
          src={pokecardMock}
      ...
      {/* Bottom sheet + Figma drawer 27084:176898 (header @30px, steps @148px from sheet top) */}
      <div
        className="absolute inset-x-0 z-[1] rounded-t-[24px] bg-white"
        style={sheetStyle}
      >
      ...
      <div
        className="absolute left-1/2 z-[10] h-[5px] w-[204px] -translate-x-1/2 rounded-[24px] bg-black"
        style={{ top: HOME_BAR_TOP }}
        aria-hidden
      />
    </div>
  )
